...
const Hero = () => {
    ...
    // Overlay + content wrapper only needed when slide has text or button
    const hasContent = Boolean(slide?.title || slide?.subtitle || slide?.buttonText || slide?.link)

    // Active side cards — when none, hero takes full width
    const activeSideCards = (settings.heroSideCards || []).filter(c => c.active !== false)
                ...
                <div className='relative flex-1 min-h-56 sm:min-h-72 rounded-3xl xl:min-h-100 group overflow-hidden bg-slate-800'>
                    {loaded ? (
                        <>
                            ...
                            {hasContent && <div className='absolute inset-0 bg-gradient-to-r from-black/70 via-black/45 to-black/10'></div>}
                            ...
                            {hasContent && (
                                <div className='relative z-10 flex flex-col justify-center h-full p-6 sm:p-12 xl:p-16'>
                                    ...
                                    {(slide?.buttonText || slide?.link) && (
                                        <Link href={slide?.link || '/shop'} className='w-fit bg-white text-slate-900 text-sm py-2.5 px-7 sm:py-4 sm:px-10 mt-4 sm:mt-8 rounded-md hover:bg-slate-100 hover:scale-103 active:scale-95 transition inline-flex items-center gap-2 font-medium'>
                                            {slide?.buttonText || t('shopNow')} <ArrowRightIcon size={16} />
                                        </Link>
                                    )}
                                </div>
                            )}
