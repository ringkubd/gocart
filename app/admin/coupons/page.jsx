...
export default function AdminCoupons() {
    ...
    const handleAddCoupon = async (e) => {
        ...
        try {
            ...
            toast.success(editingCode ? t('couponUpdated') : t('couponAdded'))
            resetForm()
            fetchCoupons()
        } catch (error) {
    ...
    return (
        <div className="text-slate-500 mb-40">
            ...
            <form onSubmit={(e) => toast.promise(handleAddCoupon(e), { loading: editingCode ? "Updating coupon..." : "Adding coupon..." })} className="max-w-sm text-sm">
                <h2 className="text-2xl">{editingCode ? t('editCoupon') : t('addCoupon')}</h2>
                {editingCode && <button type="button" onClick={resetForm} className="text-xs text-slate-400 hover:text-slate-600 underline mt-1">{t('cancelEditing')}</button>}
                <div className="flex gap-2 max-sm:flex-col mt-2">
                    <input type="text" placeholder={t('couponCodeLabel')} className="w-full mt-2 p-2 border border-slate-200 outline-slate-400 rounded-md"
                        name="code" value={newCoupon.code} onChange={handleChange} required disabled={!!editingCode}
                    />
                    <input type="number" placeholder={t('couponDiscount')} min={1} max={100} className="w-full mt-2 p-2 border border-slate-200 outline-slate-400 rounded-md"
                        name="discount" value={newCoupon.discount} onChange={handleChange} required
                    />
                </div>
                <input type="text" placeholder={t('couponDescription')} className="w-full mt-2 p-2 border border-slate-200 outline-slate-400 rounded-md"
                    name="description" value={newCoupon.description} onChange={handleChange} required
                />

                <label>
                    <p className="mt-3">{t('couponExpiry')}</p>
                ...
                <button className="mt-4 p-2 px-10 rounded bg-slate-700 text-white active:scale-95 transition">{editingCode ? t('updateCoupon') : t('addCoupon')}</button>
            </form>

            {/* List Coupons */}
            <div className="mt-14">
                <h2 className="text-2xl">{t('listCoupons')}</h2>
                {coupons.length ? (
                <div className="overflow-x-auto mt-4 rounded-lg border border-slate-200 max-w-4xl">
                    ...
                    <p className="text-slate-400 mt-3">{t('noCoupons')}</p>
                )}
            </div>
        </div>
    )
