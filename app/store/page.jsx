...
export default function Dashboard() {
    ...
    const dashboardCardsData = [
        ...
        { title: 'Total Earnings', value: currency + Number(dashboardData.totalEarnings).toLocaleString(), icon: CircleDollarSignIcon },
    ...
    return (
        <div className=" text-slate-500 mb-28">
            ...
            <div className="w-full max-w-3xl h-72 mb-10">
                <h3 className="font-medium text-slate-700 mb-4">Revenue / Day</h3>
                {chartData.length > 0 ? (
                    ...
                    dashboardData.ratings.length ? dashboardData.ratings.map((review, index) => (
                        <div key={index} className="flex max-sm:flex-col gap-5 sm:items-center justify-between py-6 border-b border-slate-200 text-sm text-slate-600 max-w-4xl">
                            ...
                            <div className="flex flex-col justify-between gap-6 sm:items-end">
                                ...
                                {review.product && <button onClick={() => router.push(`/product/${review.product.id}`)} className="bg-slate-100 px-5 py-2 hover:bg-slate-200 rounded transition-all">View Product</button>}
                            </div>
                        </div>
                    )) : (
